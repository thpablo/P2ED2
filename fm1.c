...
// Usando mememoria interna tamanho 5
// 6 fitas no total -> 5 entrada 1 saida
...
void generateOrderedBlocks(const char* dataFile){
    ...
    int count = 0;
    FILE *inputTapes[MAX_INPUT_TAPES];
    ...
    while(!feof(data)){
        ...
        fwrite(&(heap[0].regs), sizeof(Register), 1, inputTapes[count]); ///////////////
